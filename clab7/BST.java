...
public class BST<Key extends Comparable<Key>> {
    ...
    private int depth(Node x, Key key, int depth) {
        if (x == null) return depth;
        int cmp = key.compareTo(x.key);
        if      (cmp < 0) return depth(x.left, key, depth + 1);
        else if (cmp > 0) return depth(x.right, key, depth + 1);
        return depth;
    }
}

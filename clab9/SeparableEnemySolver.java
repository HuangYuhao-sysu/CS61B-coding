...
public class SeparableEnemySolver {
    ...
    public boolean isSeparable() {
        ...
        for (String n : nodes) {
            ...
            fringe.add(start); // Add to fringe.
            mark.add(start); // Mark it.
            group1.add(start);
            while (fringe.size() != 0) {
                String v = fringe.remove(0);
                for (String neighbor : g.neighbors(v)) {
                    if (!mark.contains(neighbor)) {
                        fringe.add(neighbor);
                        mark.add(neighbor);
                        if (group1.contains(v)) group2.add(neighbor);
                        if (group2.contains(v)) group1.add(neighbor);
                    }
                }
            }
            if (isConneted(group1) || isConneted(group2)) return false;
        }
        return true;
    }

    /**
     * If any two node is connectted.
     */
    private boolean isConneted(Set<String> nodes) {
        for (String node :nodes) {
            for (String neighbor : g.neighbors(node)) {
                if (nodes.contains(neighbor)) return true;
            }
        }
        return false;
    }
